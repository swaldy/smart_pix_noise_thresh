...
for sensor_geom in sizes:
        output_file = open(directory+"output_"+sensor_geom+"_"+str(noise_threshold)+"NoiseThresh.txt", "w")

        # for thresh_iter in [0.2]:
        for thresh_iter in [0.1,0.15,0.2,0.3,0.4,0.5]:
            threshold = thresh_iter
            print("Producing datasets for sensor =", sensor_geom, " noise =", noise_threshold, " thresh = ", thresh_iter)
            # Global variables
            ...
            dirtrain = '/eos/user/s/swaldych/smart_pix/'+train_dataset_name+'/'+train_dataset_name+'_'+sensor_geom+'_parquets/unflipped/'
            ...
            dirtest = '/eos/user/s/swaldych/smart_pix/'+test_dataset_name+'/'+test_dataset_name+'_'+sensor_geom+'_parquets/unflipped/'
            ...
            plt.title('pT of all events')
            plt.savefig(directory+"test_pt_all_"+sensor_geom+"_0P"+str(threshold - int(threshold))[2:]+"thresh_"+str(noise_threshold)+"NoiseThresh.png")
            plt.close()

            plt.hist(testlabels_csv[abs(testlabels_csv['pt'])>threshold]['pt'], bins=100)
            plt.title('pT of Class 0 events')
            plt.savefig(directory+"test_pt_cls0_"+sensor_geom+"_0P"+str(threshold - int(threshold))[2:]+"thresh_"+str(noise_threshold)+"NoiseThresh.png")
            plt.close()
            ...
            plt.title('pT of Class 1+2 events')
            plt.savefig(directory+"test_pt_cls12_"+sensor_geom+"_0P"+str(threshold - int(threshold))[2:]+"thresh_"+str(noise_threshold)+"NoiseThresh.png")
            plt.close()
